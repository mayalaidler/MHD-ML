...
def temp_spread_over_time():
    ...
    hydro_path = "/scratch/ebuie/ISO_Turb/midway/1E23_S100_z1/"
    name = "1E23_S100_z1_mhd"
    hydro_name = "1E23_S100_z1 (Hydro run)"
    ...
    plt.plot(uvrad, stds[7], 'o', label='Background Radiation equilibrium', color='tab:red', markersize=10)
    plt.xlabel("Time (Myr)")
    plt.ylabel("Temp (K)")
    plt.title(f"Temperature Evolution for {hydro_name}")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("hydro_temp_spread_over_time.png", dpi=200)
    plt.close()

    print("Saved temp_spread_over_time_hydro.png and data file.")
